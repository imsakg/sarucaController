...
def global_stretching(r_array, height, width, lamda, k):
    ...
    if Dif >= 1:
        ...
        for i in range(0, height):
            for j in range(0, width):
                if r_array[i][j] < I_min:
                    ...
                    p_out = (
                        int((r_array[i][j] - I_min) * ((255 - I_min) / (I_max - I_min)))
                        + I_min
                    )
                    array_Global_histogram_stretching[i][j] = p_out
    else:
        ...
        if r_array[i][j] < I_min:
            ...
            p_out = (r_array[i][j] - np.min(r_array)) * (
                DR_min / np.min(r_array)
            ) + np.min(r_array)
            array_Global_histogram_stretching[i][j] = p_out
        else:
            p_out = (
                int((r_array[i][j] - I_min) * ((255 - DR_min) / (I_max - I_min)))
                + DR_min
            )
            array_Global_histogram_stretching[i][j] = p_out
...
def RelativeGHstretching(sceneRadiance, height, width):
    ...
    sceneRadiance[:, :, 0] = global_stretching(
        sceneRadiance[:, :, 0], height, width, 0.97, 1.25
    )
    sceneRadiance[:, :, 1] = global_stretching(
        sceneRadiance[:, :, 1], height, width, 0.95, 1.25
    )
    sceneRadiance[:, :, 2] = global_stretching(
        sceneRadiance[:, :, 2], height, width, 0.83, 0.85
    )

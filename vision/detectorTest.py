...
def detection(imageFrame):
	...
	for pic, contour in enumerate(contours):
		...
		if (area>30) and (n>10):
			...
			return imageFrame, [x,y,w,h], res_blue, erode, dilate,blue_mask,cv2.drawContours(np.zeros(imageFrame.shape),contours,pic,255,-1),thresh,morph
	return imageFrame,None, res_blue, erode, dilate,blue_mask,np.zeros(imageFrame.shape),thresh,morph

while True:
	_,img = cap.read()
	img, coords, res_blue, erode, dilate,blue_mask, contours,thresh,morph = detection(img)
